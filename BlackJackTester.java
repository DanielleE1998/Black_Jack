...
public class BlackJackTester extends BlackJackApp {
	...
	private static boolean blackJackClass() {
		...
		// test static method getValueOfHand
		Hand h = new Hand();
		...
		h = new Hand();
		h.addCard(new Card(Card.Type.QUEEN, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.ACE, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.FIVE, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.SIX, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.SEVEN, Card.Suit.SPADE));
		if(BlackJack.getValueOfHand(h) != 29)
		{
			System.out.println("The value of the hand is not correct");
			System.out.println("Your calculated value is: " + BlackJack.getValueOfHand(h));
			System.out.println("The actual value is: 20");
		...
		h = new Hand();
		h.addCard(new Card(Card.Type.QUEEN, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.KING, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.EIGHT, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.NINE, Card.Suit.SPADE));
		h.addCard(new Card(Card.Type.TEN, Card.Suit.SPADE));
		if(BlackJack.getValueOfHand(h) != 47)
		{
			System.out.println("The value of the hand is not correct");
			System.out.println("Your calculated value is: " + BlackJack.getValueOfHand(h));
			System.out.println("The actual value is: 20");
		...
		// make sure blackjack starts both player and dealer with two cards
		BlackJack game = new BlackJack();
		
		if ((game.getDealer().getHand().size() !=2) ||
				(game.getPlayer().getHand().size() !=2)	){
			System.out.println("Dealer or Player hand do not start with 2 cards");
			System.out.println("Dealer hand has "+ game.getDealer().getHand().size());
			System.out.println("Player hand has "+ game.getPlayer().getHand().size());
			return false;
		}
		
		//make sure blackJack restart method resets game properly
		// add a few cards to both player and dealer
		game.getDealer().playTurn(game.getDeck());
		game.getDealer().playTurn(game.getDeck());
		game.getDealer().playTurn(game.getDeck());
		
		// verify dealer has 5 cards
		if (game.getDealer().getHand().size()!=5){
			System.out.println("Dealer has "+ game.getDealer().getHand().size() +" cards instead of 5");
			return false;
		}
		...
		c = game.getDeck().draw();
		game.getPlayer().getHand().addCard(c);
		if (game.getPlayer().getHand().size()!=4){
			System.out.println("Player has "+ game.getPlayer().getHand().size() +" cards instead of 4");
			return false;
		}
		...
		if ((game.getDealer().getHand().size() !=2) ||
				(game.getPlayer().getHand().size() !=2)	){
			System.out.println("Dealer or Player hand do not restart with 2 cards");
			System.out.println("Dealer hand has "+ game.getDealer().getHand().size());
			System.out.println("Player hand has "+ game.getPlayer().getHand().size());
			return false;
		}
		
		// make sure that there are 52 cards in the deck (at this point 48 left)
		
		for (int i=0;i<48;i++){
			if (game.getDeck().draw() == null){
				System.out.println("deck run ut of cards after " +(i+4) + " drawa");
				return false;
			}
			
		}
		// make sure now deck is empty
		if (game.getDeck().draw() != null){
			System.out.println("deck contains more than 52 cards");
			return false;
		}
