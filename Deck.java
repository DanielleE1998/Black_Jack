...
public class Deck {
	...
	public Card draw() {
		Card card = null;
		System.out.println("in draw number of cards is "+numCardsInDeck);
		if (numCardsInDeck>0){
			card = cards[numCardsInDeck-1]; //get the last card
			numCardsInDeck--;
		}
		return card;
	}
	...
	private void shiftRightOne() {
		if (numCardsInDeck == 52){
			...
			for (int i=numCardsInDeck-1;i>-1;i--){
				cards[i+1]=cards[i];
			}
		}
	}	
	...
	public void addToBottom(Card c) {
		System.out.println("in addToBOtton number of cards is "+numCardsInDeck);
		if (numCardsInDeck == 52){
			System.out.println("Deck is full");
		}
		else{
	...
	public void shuffle(int n) {
		...
		for (int i=0;i<n;i++){
			int intVal1 = rn.nextInt(52); // get a random int between 0 and 51
			int intVal2 = rn.nextInt(52); // get a random int between 0 and 51
